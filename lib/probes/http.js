    ...
    proxy.before(obj.Server.prototype, ['on', 'addListener'], function(obj, args) {
        ...
        proxy.callback(args, -1, function(obj, args) {
            ...
            req.graphdat = {
                ...
                begin: function(name) {
                    if(gd.debug && gd.debug.http_begin) gd.gdLog(gd.logType.Info, 'begin %s', name);
                    req.graphdat.context.enter();
                },
                end: function(name) {
                    var data = req.graphdat.context.leave(name);
                    if(gd.debug && gd.debug.http_end) gd.gdLog(gd.logType.Info, 'end %s (%dms)', name, data.time.ms);
                },
	            trace: function(callback) {
		            if(gd.debug && gd.debug.http_trace) gd.gdLog(gd.logType.Info, 'trace in');
