...
var start = function() {
	...
	gd.disabledProbes = {};
	gd.options = {};
	gd.socket = process.platform == 'windows' ? '127.0.0.1:26873' : '/tmp/gd.agent.sock';
...
var optionsReport = function() {
	...
	if(gd.enabled || gd.debug) {
		gd.gdLog(gd.logType.Info, "Sending to agent on %s", gd.socket);
	}
}
	...
	if(typeof opt !== 'undefined') {
		...
		if(process.platform == "windows") {
			if(typeof opt.port !== 'undefined') {
				this.socket = '127.0.0.1:' + opt.port;
			}
		}
		else {
			if(typeof opt.socketFile !== 'undefined') {
				this.socket = opt.socketFile;
			}
		}
	}
	optionsReport();
