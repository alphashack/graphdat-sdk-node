...
node.prototype = {
    _logAndThrow: function(ex) {
        if(this._context._logger) {
	        this._context._logger(ex);
        }
        throw ex;
    },
	enter: function(name) {
		if(name && this._name != name) this._logAndThrow('Context error: tried to (re)enter "' + name + '" but current context is "' + this._name + '"');
